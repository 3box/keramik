...
pub fn scenario(topo: Topology) -> Result<Scenario, GooseError> {
    ...
    let test_start = Transaction::new(Arc::new(move |user| {
        Box::pin(setup(topo, user, setup_cli.clone()))
    }))
    .set_name("setup")
    .set_on_start();

    let update_small_model =
        Transaction::new(Arc::new(move |user| Box::pin(update_small_model(user))))
            .set_name("update_small_model");

    let get_small_model = Transaction::new(Arc::new(move |user| Box::pin(get_small_model(user))))
        .set_name("get_small_model");

    let update_large_model =
        Transaction::new(Arc::new(move |user| Box::pin(update_large_model(user))))
            .set_name("update_large_model");

    let get_large_model = Transaction::new(Arc::new(move |user| Box::pin(get_large_model(user))))
        .set_name("get_large_model");
...
async fn setup(_topo: Topology, user: &mut GooseUser, cli: CeramicHttpClient) -> TransactionResult {
...
async fn update_small_model(user: &mut GooseUser) -> TransactionResult {
    let (model, url, req) = {
        ...
        let req = GooseRequest::builder()
            ...
            .set_request_builder(user.client.get(url.clone()))
