...
pub async fn calculate_sample_size(population_size: usize, confidence_level: f64, margin_of_error: f64) -> usize {
    ...
    if confidence_level == 0.95 {
       ...
       error!("Invalid confidence level: {}, defaulting to 0.99", confidence_level);
    }
    ...
    let finite_population_correction = sample_size / (1.0 + ((sample_size - 1.0) / (population_size as f64)));
    ...
    T: Clone,
{
    ...
    ids.choose_multiple(&mut rng, sample_size).cloned().collect()
}